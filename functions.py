...
def thermoelectric_efficiency_estimator(T_C, T_H):
    CtoK = 273.15
    T_ave = (T_H + T_C) / 2
    DT = T_H - T_C
    ...
    # Calculation based on Zeta values for p-PbTe
    S_P = 0.34987391 * T_ave + 114.4786318
    R_P = (-1e-8 * T_ave**3 + 1e-5 * T_ave**2 + 0.0017 * T_ave + 0.6186)  # Resistivity [mOhm * cm]
    K_P = 8e-6 * T_ave**2 - 0.007 * T_ave + 2.6118  # Thermal Conductivity [W/m/K]
    ZeTa_P = S_P**2 / R_P / K_P * (T_ave + CtoK) / 1e7
    maxRedEff_P = (np.sqrt(1 + ZeTa_P) - 1) / (np.sqrt(1 + ZeTa_P) + 1)
    # print("T : ", T_ave, " | Seebeck : ", S_P, "| R_TEG : ", R_P, "| K_TEG : ", K_P, "| ZeTa : ", ZeTa_P, "| maxRedEff : ", maxRedEff_P)

    # Calculation based on Zeta values for n-PbTe
    S_N = -0.293284981 * T_ave + -78.40910427
    R_N = (-2e-9 * T_ave**3 + 8e-6 * T_ave**2 + 0.001 * T_ave + 0.2729)  # Resistivity [mOhm * cm]
    K_N = 1e-5 * T_ave**2 - 0.0096 * T_ave + 3.4338  # Thermal Conductivity [W/m/K]
    ZeTa_N = S_N**2 / R_N / K_N * (T_ave + CtoK) / 1e7
    maxRedEff_N = (np.sqrt(1 + ZeTa_N) - 1) / (np.sqrt(1 + ZeTa_N) + 1)
    # print("T : ", T_ave, " | Seebeck : ", S_N, "| R_TEG : ", R_N, "| K_TEG : ", K_N, "| ZeTa : ", ZeTa_N, "| maxRedEff : ", maxRedEff_N)

    # Calculation of Zeta & max Efficiency based on PN junction semiconductor Seebeck effect
    ...
    # print("T : ", T_ave, " | ZT_PN : ", ZT_PN, "| Eta_Max : ", Eta_Max, "| EtaMaxEff : ", Eta_Max_Eff, " | Eta_Carnot : ", Eta_Carnot)
    print("WARNING: there is sth wring with fromula so Carnot/5 used as eta temporarily")
    """WARNING- there is sth wring with fromula so Carnot/5 used as eta temporarily"""
    return Eta_Carnot/5

# Example usage
# thermoelectric_efficiency_estimator(5, 40)
# print(thermoelectric_efficiency_estimator(25,280))
