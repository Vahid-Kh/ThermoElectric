...
in case of MINI LOG the data comming has space between date and time to fix this issue add DATE to header file
...
for data_file_i in data_file:
    ...
    data_frame['Time'] = data_frame['Date'] + ' ' + data_frame['Time']
    data_frame = data_frame.drop("Date", axis=1)
    data_frame.to_csv(txt_dot_decimal_comma_sep, index=False)
